...
// Main React entry point. This file owns the whole single-page UI: chat,
// agent selection, document upload, structured JSON output, trace, and insights.
const API_BASES = ["", "http://127.0.0.1:5000"];

// Calls the local Express API. The empty base works through the Vite proxy;
// the localhost fallback helps when the built frontend is served separately.
...
// Root application component for the Azure AI Foundry service-agent console.
function App() {
  ...
  // Load backend runtime settings and available agents when the UI starts.
  useEffect(() => {
    apiFetch("/api/config")
      .then(({ payload }) => {
        setConfig(payload);
  ...
  // Sends a normal chat message to the backend and renders the agent response.
  async function sendMessage(event) {
    ...
    try {
      ...
      if (payload.agent?.key) {
        setAgentKey(payload.agent.key);
      }
  ...
  // Starts a fresh browser session and clears related backend thread/document state.
  ...
  // Uploads a document so later chat and structured requests can use its text.
  ...
  // Requests machine-readable JSON using one of the configured output schemas.
